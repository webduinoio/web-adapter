  ...
  transform(chunk, controller) {
    ...
    if (this.readByteArray) {
      ...
      if (endSign[0] == 4 && endSign[1] == 4 && endSign[2] == 62) {
        var allData = new Uint8Array([
          ...this.byteArray.slice(0, byteArrayLength - 3),
        ]);
        var rtnData = [];
        var start = false;
        // find startPos '>OK'
...
class REPL {
  ...
  async usbConnect(board = "esp32cam") {
    var self = this;
    ...
    if (self.port == undefined) {
      ...
      this.reader = this.port.readable
        .pipeThrough(new TransformStream(this.stream))
        .getReader();
  ...
  async waitRestart() {
    ...
    while (1) {
      ...
      // console.log("value:" + value+",done:"+done+",len:"+value.length);
      if (value.substring(0, 35) === 'Type "help()" for more information.') {
        break;
      }
    }
    console.log("restart ok");
  ...
  async snapshot(board) {
    this.stream.setReadLine();
    var code = board == "k210" ? snapshotK210Code : snapshotESP32Code;
    //console.log(">>>", code);
    var imgSize = await this.write(code, async function (value) {
    ...
    await this.writer.write(Int8Array.from([0x01 /*RAW paste mode*/]));
    var cmd = board == "k210" ? "repl.write(jpg)" : "sys.stdout.write(jpg)";
